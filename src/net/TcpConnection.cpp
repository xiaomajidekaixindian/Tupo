...
TcpConnection::TcpConnection(int connfd, EventLoop *loop,
    ...
    : socket_(std::make_unique<Socket>(connfd)), loop_(loop),
      ...
      channel_(std::make_unique<Channel>(loop, connfd)), state_(kConnecting),
      connfd_(connfd), inputBuffer_(std::make_unique<Buffer>()),
...
void TcpConnection::send(const std::string &msg) {
  if (state_ == kConnected) {
    if (loop_->isInLoopThread()) {
      ...
      loop_->runInLoop([this, msg] { this->sendInLoop(msg); });
    }
  }
}
...
void TcpConnection::send(const char *data, size_t len) {
  if (state_ == kConnected) {
    if (loop_->isInLoopThread()) {
      ...
      loop_->runInLoop([this, data, len] { this->sendInLoop(data, len); });
    }
  }
}
...
void TcpConnection::send(Buffer &&buffer) {
  if (state_ == kConnected) {
    if (loop_->isInLoopThread()) {
      ...
      loop_->runInLoop([this, data] { this->sendInLoop(*data); });
    }
  }
}
...
void TcpConnection::handleWrite() {
  if (channel_->isWriting()) {
    ssize_t n = ::write(connfd_, outputBuffer_->readablePtr(),
